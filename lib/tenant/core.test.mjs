...
import { setRecurringPayment, setBankConnected, updateDisplayName, acknowledgeMoveIn, acknowledgeMoveOut } from "./core.js";
...
test("acknowledgeMoveIn stamps the confirmation timestamp", async () => {
  const a = createFakeTenantAdapter({ profiles: [{ id: "t1" }] });
  const at = await acknowledgeMoveIn(a, "t1", { now: "2026-08-03T17:00:00Z" });
  assert.equal(at, "2026-08-03T17:00:00Z");
  assert.equal(a._store.profiles[0].move_in_acked_at, "2026-08-03T17:00:00Z");
});

test("acknowledgeMoveIn is idempotent — a second click keeps the first timestamp", async () => {
  const a = createFakeTenantAdapter({ profiles: [{ id: "t1", move_in_acked_at: "2026-07-01T09:00:00Z" }] });
  const at = await acknowledgeMoveIn(a, "t1", { alreadyAckedAt: "2026-07-01T09:00:00Z", now: "2026-08-03T17:00:00Z" });
  assert.equal(at, "2026-07-01T09:00:00Z");
  assert.equal(a._store.profiles[0].move_in_acked_at, "2026-07-01T09:00:00Z");
});

test("acknowledgeMoveOut stamps its own column, not the move-in one", async () => {
  const a = createFakeTenantAdapter({ profiles: [{ id: "t1" }] });
  await acknowledgeMoveOut(a, "t1", { now: "2026-08-03T17:00:00Z" });
  assert.equal(a._store.profiles[0].move_out_acked_at, "2026-08-03T17:00:00Z");
  assert.equal(a._store.profiles[0].move_in_acked_at, undefined);
});
