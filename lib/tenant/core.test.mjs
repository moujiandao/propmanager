...
test("mapTenant normalizes status and maps snake->camel", () => {
  const row = {
  ...
  assert.equal(t.status, "current tenant");      // active -> current tenant
  ...
  assert.equal(mapTenant({ id: "t2", status: "inactive" }).status, "previous tenant");
  assert.equal(mapTenant({ id: "t3" }).status, "current tenant"); // default
});
