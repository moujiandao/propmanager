...
const COL_W = 90;
// Ceiling on a diagonal stall's lean, for the degenerate case below. Past a
// shear equal to the stall's own depth the parallelogram stops reading as a
// stall at all, so the angle gives way before the shape does.
const MAX_SKEW = COL_W;
const BACK_W = 70;
const BACK_H = 55;
...
const RIGHT_EDGE = OUTLINE.x + OUTLINE.w - INSET;
...
//
// The shift equals the stall's own height, which is what makes the angle 45°:
// neighbouring stalls share a slanted edge, and that edge advances one unit
// across for every unit down. A fixed shift can't hold the angle, because the
// height is divided among however many spots the zone has -- the old constant
// 22 against a 7-spot height of ~34 drew ~57°, which read as barely-tilted
// boxes rather than angled parking.
//
// Degenerate case: with very few spots the stalls get tall enough that a 45°
// shear would be wider than the stall is deep, so the lean caps at MAX_SKEW
// and the angle steepens past 45° rather than the shape falling apart.
//
// x is derived per-stall rather than hoisted to a constant because it depends
// on the skew: anchoring from the right edge and pulling in by the lean is what
// keeps the widest point (the top-right corner) inside the lot.
const rightCorners = (top, bottom, h) => {
  const skew = Math.min(h, MAX_SKEW);
  const x = RIGHT_EDGE - COL_W - skew;
  return [
    [x + skew, top],
    [x + skew + COL_W, top],
    [x + COL_W, bottom],
