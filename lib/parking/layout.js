...
const WIDTH = 420;
// Taller than the lot itself: the back row parks above it and the entrance
// marker sits below it, both outside the boundary.
const HEIGHT = 362;
...
// spends this much of the lot's *height* on the stripes' climb.
const RIGHT_DEPTH = 60;
const AISLE_OFFSET = 34; // how far left of the bank the direction arrow sits
const BACK_W = 70;
const BACK_H = 55;
...
const ARROW_X = AISLE_X - AISLE_OFFSET;
...
export function lotLayout(spots) {
  ...
  return {
    ...
    outline: { ...OUTLINE },
    stalls,
    // Traffic runs up the aisle from the entrance. Drawn only when there is an
    // angled bank to run alongside -- an aisle pointing at nothing is noise.
    aisle: zones.right.length
      ? {
          x: ARROW_X,
          fromY: COL_BOTTOM,
          toY: COL_TOP + 16,
          head: toPoints([
            [ARROW_X, COL_TOP],
            [ARROW_X - 7, COL_TOP + 16],
            [ARROW_X + 7, COL_TOP + 16],
          ]),
        }
      : null,
    entrance: { x: ARROW_X, y: OUTLINE.y + OUTLINE.h + 20 },
