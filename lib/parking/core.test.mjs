...
test("car fields are optional, trimmed, and year is coerced to a number", async () => {
  const a = createFakeParkingAdapter();

  await createSpot(a, {
    landlordId: "L1", propertyId: "P1", label: "1",
    carMake: "  Honda  ", carModel: "  Civic  ", carYear: "2019",
  });
  const row = a._store.parking_spots[0];
  assert.equal(row.car_make, "Honda");
  assert.equal(row.car_model, "Civic");
  assert.equal(row.car_year, 2019);
  assert.equal(typeof row.car_year, "number");

  // Omitted entirely -> all null, not undefined or empty string.
  await createSpot(a, { landlordId: "L1", propertyId: "P1", label: "2" });
  assert.deepEqual(
    { make: a._store.parking_spots[1].car_make, model: a._store.parking_spots[1].car_model, year: a._store.parking_spots[1].car_year },
    { make: null, model: null, year: null },
  );

  // A non-numeric year must not reach the database as NaN.
  await createSpot(a, { landlordId: "L1", propertyId: "P1", label: "3", carYear: "not a year" });
  assert.equal(a._store.parking_spots[2].car_year, null, "NaN -> null");
});
...
test("updateSpot edits label/type/rate, trims, and rejects a blank label", async () => {
  ...
  await updateSpot(a, "s1", { label: "  1A  ", type: "  left side (straight)  ", monthlyRate: "80", carMake: "Toyota", carModel: "Corolla", carYear: "2021" });
  const row = a._store.parking_spots[0];
  assert.equal(row.label, "1A");
  assert.equal(row.type, "left side (straight)");
  assert.equal(row.monthly_rate, 80);
  assert.equal(typeof row.monthly_rate, "number");
  assert.equal(row.car_make, "Toyota");
  assert.equal(row.car_year, 2021);
