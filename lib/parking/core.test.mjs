...
import { createSpot, updateSpot, deleteSpot, createLease, updateLease, deleteLease, endLease } from "./core.js";
import { mapParkingSpot, mapParkingRenter, mapParkingLease } from "./mappers.js";
import { isActiveLease, isSpotLeased, activeLeaseForSpot, leaseParty, SPOT_TYPES, isValidSpotType } from "./status.js";

test("createSpot writes snake_case fields", async () => {
  ...
  // The spot describes physical reality only: money, vehicle and who it is let
  // to all live on the lease.
  assert.equal("market_status" in row, false);
...
test("updateLease edits both dates, the rate, and the vehicle", async () => {
  const a = createFakeParkingAdapter({
    parking_leases: [{ id: "l1", rate: 80, start_date: "2026-01-01", end_date: null, car_make: "Honda", car_model: "Civic", car_year: 2019 }],
  });

  await updateLease(a, "l1", {
    rate: "95", startDate: "2026-02-01", endDate: "2027-01-31",
    carMake: "  Toyota  ", carModel: "Corolla", carYear: "2021",
  });
  const lease = a._store.parking_leases[0];
  assert.equal(lease.rate, 95);
  assert.equal(typeof lease.rate, "number");
  assert.equal(lease.start_date, "2026-02-01");
  ...
  await updateLease(a, "l1", { rate: "95", startDate: "2026-02-01", endDate: "", carMake: "", carModel: "", carYear: "" });
...
test("updateLease rejects what the database's NOT NULLs and CHECK would reject", async () => {
  const a = createFakeParkingAdapter({
    parking_leases: [{ id: "l1", rate: 95, start_date: "2026-02-01", end_date: null }],
  });
  const good = { rate: "95", startDate: "2026-02-01" };

  // rate and start_date are NOT NULL; end_date >= start_date is a CHECK. Failing
  // here rather than at the database is what lets a caller show a real message
  // instead of a raw constraint string.
  await assert.rejects(() => updateLease(a, "l1", { ...good, rate: "" }), /rate is required/);
  await assert.rejects(() => updateLease(a, "l1", { ...good, rate: "0" }), /rate is required/);
  await assert.rejects(() => updateLease(a, "l1", { ...good, startDate: "" }), /startDate is required/);
  await assert.rejects(
    () => updateLease(a, "l1", { ...good, endDate: "2026-01-31" }),
    /endDate cannot precede startDate/,
  );

  // An end date equal to the start is fine -- a one-day lease is a lease.
  await updateLease(a, "l1", { ...good, endDate: "2026-02-01" });
  assert.equal(a._store.parking_leases[0].end_date, "2026-02-01");

  assert.deepEqual(
    { rate: a._store.parking_leases[0].rate, start: a._store.parking_leases[0].start_date },
    { rate: 95, start: "2026-02-01" },
    "every rejected edit left the row untouched",
  );
});

test("deleteLease removes the row; unknown id throws", async () => {
  const a = createFakeParkingAdapter({ parking_leases: [{ id: "l1" }, { id: "l2" }] });
  await deleteLease(a, "l1");
  assert.deepEqual(a._store.parking_leases.map((l) => l.id), ["l2"], "only the named lease goes");
  await assert.rejects(() => deleteLease(a, "missing"), /not found/);
});
...
test("mappers map snake_case to camelCase", () => {
  ...
  assert.equal("marketStatus" in spot, false, "who a spot is let to is derived from its lease, not stored");
...
test("leaseParty reads who a spot is let to off the lease itself", () => {
  // The lease's CHECK guarantees exactly one of tenant_id/renter_id, so this
  // cannot be ambiguous the way the old stored market_status could.
  assert.equal(leaseParty({ tenantId: "t1", renterId: null }), "tenant");
  assert.equal(leaseParty({ tenantId: null, renterId: "r1" }), "renter");
  assert.equal(leaseParty(null), null, "a vacant spot is let to nobody");
  assert.equal(leaseParty(undefined), null);
  assert.equal(leaseParty({ tenantId: null, renterId: null }), null, "a lease for neither is not a party");
});

test("a spot's label follows its CURRENT lease, which is what the stored flag got wrong", () => {
  // The bug this replaced: a spot showing "Open Market" while its live lease was
  // to a tenant. Derived from the active lease, the label cannot say one thing
  // while the lease says another.
  const today = new Date("2026-08-12T00:00:00");
  const leases = [
    { id: "old", tenantId: null, renterId: "r1", startDate: "2025-01-01", endDate: "2025-12-31" },
    { id: "now", tenantId: "t1", renterId: null, startDate: "2026-01-01", endDate: null },
  ];
  assert.equal(leaseParty(activeLeaseForSpot(leases, today)), "tenant");
  // ...and once that lease ends, the spot is let to nobody rather than keeping a stale label.
  assert.equal(leaseParty(activeLeaseForSpot([leases[0]], today)), null);
});
