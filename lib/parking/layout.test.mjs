...
test("an angled bay is striped at 30° to the aisle, between vertical aisle and wall edges", () => {
  const { stalls } = lotLayout(RIDGE_RD);
  ...
  for (const edge of slanted) {
    assert.ok(Math.abs(stripeAngle(edge) - 30) < 0.01, `stripe should meet the aisle at 30°, got ${stripeAngle(edge).toFixed(2)}°`);
  }
  ...
  for (const n of [1, 4, 7, 12]) {
    ...
    for (const s of stalls) {
      for (const edge of edgesOf(s).filter(([x]) => Math.abs(x) >= 0.01)) {
        assert.ok(Math.abs(stripeAngle(edge) - 30) < 0.01, `${n} spots: stripe is ${stripeAngle(edge).toFixed(2)}°, not 30°`);
      }
    }
...
test("an angled bay is wide enough not to read as a sliver", () => {
  ...
  const pitch = climb - depth / Math.tan((30 * Math.PI) / 180);
...
test("straight stalls keep their size when the lot stretches for the angled bank", () => {
  // The lot is long because a 30° bank eats length, not because the head-in
