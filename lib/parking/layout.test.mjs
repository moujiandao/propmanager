...
test("diagonal stalls are parallelograms with a visible slant; straight ones are rectangles", () => {
  ...
  const pts = right.points.split(" ").map((p) => p.split(",").map(Number));
  const height = Math.max(...pts.map(([, y]) => y)) - Math.min(...pts.map(([, y]) => y));
  // 45° means the shared edge between two neighbours advances exactly as far
  // across as it does down, so the horizontal shift equals the stall's height.
  assert.ok(Math.abs(skew - height) < 0.01, `expected a 45° lean, got skew ${skew} against height ${height}`);

  const left = stalls.find((s) => s.zone === "left");
  assert.equal(topX(left), bottomX(left), "straight stalls have no slant");
});

test("the diagonal lean holds at 45° whatever the spot count, and caps before the shape breaks", () => {
  const geom = (n) => {
    const spots = Array.from({ length: n }, (_, i) => ({ id: `r${i}`, label: String(i + 1), type: RIGHT }));
    const s = lotLayout(spots).stalls[0];
    const pts = s.points.split(" ").map((p) => p.split(",").map(Number));
    const minY = Math.min(...pts.map(([, y]) => y));
    const maxY = Math.max(...pts.map(([, y]) => y));
    const topX = Math.min(...pts.filter(([, y]) => y === minY).map(([x]) => x));
    const bottomX = Math.min(...pts.filter(([, y]) => y === maxY).map(([x]) => x));
    return { skew: topX - bottomX, height: maxY - minY };
  };

  // A fixed skew could only be 45° at one particular count; this is the
  // regression that change was making.
  for (const n of [4, 7, 12]) {
    const { skew, height } = geom(n);
    assert.ok(Math.abs(skew - height) < 0.01, `${n} spots: skew ${skew} != height ${height}`);
  }

  // Too few spots to hold the angle: the lean stops growing rather than
  // shearing the stall into a sliver.
  const { skew, height } = geom(1);
  assert.ok(skew < height, "a single tall stall caps its lean instead of matching its height");
});
