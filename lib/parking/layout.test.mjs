...
test("an angled bay is wide enough not to read as a sliver", () => {
  ...
  // A bay is a car-shaped box: deeper than it is wide, but not by so much that
  // it reads as a sliver. Real angled parking runs roughly 0.5-0.6 of pitch per
  // unit of depth (stall width / sin(angle), against stall depth). This guards
  // that proportion rather than either raw number, so the two stay free to be
  // retuned together -- pinning the constants would just re-break on every
  // resize, which is what the first version of this test did.
  const ratio = pitch / depth;
  assert.ok(ratio > 0.4 && ratio < 0.9, `bay pitch:depth should sit near real parking, got ${ratio.toFixed(2)}`);
});
...
test("straight stalls keep their size when the lot stretches for the angled bank", () => {
  ...
  // stalls want to be ribbons -- five of them would stretch to fill whatever the
  // bank happens to need.
  const { stalls, outline } = lotLayout(RIDGE_RD);
