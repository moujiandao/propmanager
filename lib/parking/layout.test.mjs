...
test("the aisle points away from the entrance, and only exists when there is a bank to serve", () => {
  const { aisle, entrance, outline } = lotLayout(RIDGE_RD);
  assert.ok(aisle, "a lot with angled bays gets an aisle");
  assert.ok(aisle.toY < aisle.fromY, "traffic runs up the lot, away from the entrance");
  assert.ok(aisle.x > outline.x && aisle.x < outline.x + outline.w, "the aisle runs inside the lot");
  assert.ok(entrance.y > outline.y + outline.h, "the entrance marker sits below the lot, where cars come in");

  const straightOnly = lotLayout([{ id: "l1", label: "1", type: LEFT }]);
  assert.equal(straightOnly.aisle, null, "no angled bank, no aisle arrow");
});
