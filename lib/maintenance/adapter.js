...
export function createSupabaseAdapter(supabase, {
  ...
  return {
    ...
    async deleteRequest(id) {
      ...
      ok(error, "deleteRequest");
    },
    // Attachment rows cascade with the request; their files do not. Same
    // "documents" bucket AttachmentChip reads signed URLs from.
    async removeFiles(paths) {
      const { error } = await supabase.storage.from("documents").remove(paths);
      ok(error, "removeFiles");
    },
