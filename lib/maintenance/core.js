...
// Set a request's workflow status. Returns the id+status that was written.
export async function setStatus(adapter, id, status) {
  await adapter.updateRequest(id, { status });
  return { id, status };
}
