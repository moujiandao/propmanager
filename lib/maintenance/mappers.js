...
export const mapMaintenance = (m) => ({
  ...
  // Date-only, same treatment as `date`, so fmtDate's YYYY-MM-DD → local-midnight
  // rule applies and a ticket closed late in the evening doesn't display as the
  // next day. Null on open tickets and on ones closed before the column existed.
  closedDate: m.closed_at ? m.closed_at.split("T")[0] : null,
  // Full timestamp kept for the write path: setStatus needs the existing value
  // to avoid rewriting the close date when a closed ticket is re-closed.
