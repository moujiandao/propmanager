...
async function loadLandlordData(supabase) {
  const [
    ...
    // maybeSingle, not single: a landlord who has never opened Payment
    // Reminders has no email_settings row, and that is a normal state —
    // mapEmailSettings(null) returns the defaults. `.single()` treats zero rows
    // as an error, so every page load logged a 406 that nothing acted on.
...
async function loadTenantData(supabase, user) {
  // maybeSingle for the same reason: the `tenant ? ... : null` handling below
  // already treats a missing profile as a real state, so there is no value in
  // PostgREST raising on it first.
  const { data: tenRow } = await supabase.from('tenant_profiles').select('*').eq('id', user.id).maybeSingle();
