...
test("candidates(move_in): anyone not yet departed who has a move-in date", () => {
  // Deliberately NOT gated on "future tenant". Status derives from the dates, so a
  // tenant becomes "current" at midnight on their move-in day — gating on future
  // would make an offset-0 move-in reminder find nobody by the time the cron runs.
  // t4 has no move-in date and t5 has already left; t1/t2 are past move-ins, which
  // are harmless here because offsetsDueToday() rejects negative lead times.
  assert.deepEqual(ids(candidates("move_in", ds)), ["t1", "t2", "t3"]);
});
