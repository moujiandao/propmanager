...
### Fixed
- Every page load logged a `406` from `email_settings`. The read used `.single()`, which treats zero rows as an error, but a landlord who has never opened Payment Reminders legitimately has no row — `mapEmailSettings(null)` already returns the defaults. Now `.maybeSingle()`, as is the tenant-profile read for the same reason. `scripts/fix-email-settings-defaults.sql` additionally corrects the column defaults: four of the five reminder flags defaulted to `true`, so any row created *outside* the app (a seed script, a manual INSERT, a future route inserting a partial row) would switch on reminder emails nobody enabled. The app's own write path already specifies every flag, so this closes the remaining doors rather than fixing a live path.
- The tenant detail page's "Tenant not found." and "← Back" were hardcoded English, so they stayed English for a Chinese-language landlord. Now `t.tenantNotFound` / `t.unitBack`, in both locales.
- **Payment Reminders could never save, and the toggle lied about it.** Every write omitted `landlord_id`, so the row failed `is_team_member(null)` and came back 403 — the toggle flipped optimistically and then silently reverted a second later. The adapter's comment asserted "the row's landlord_id is set by the DB/RLS"; RLS *checks* `landlord_id`, it does not supply one, and there is no column default. It also meant `onConflict: landlord_id` could never match an existing row, so every write was an insert. `createPaymentReminderAdapter` now requires the **team** id (`user.id`, never `authId`) and stamps it into every patch. The in-memory fake requires and stamps it too: the old fake accepted a patch without it, so it agreed with the core and disagreed with the database, which is how a write path that could not succeed had passing tests.
- **Enabling one payment reminder silently enabled three others.** `setReminderField` sent only the changed column, so on the write that *creates* the row the remaining columns fell to their database defaults — and those defaults are inconsistent and mostly `true` (`day_of_reminder`, `three_day_overdue`, `seven_day_overdue` default true; `one_day_overdue` false). Since this feature emails tenants, that is a live hazard, not a cosmetic bug. Writes now specify every flag, so the row is fully determined by what the landlord set and the operation is idempotent regardless of defaults. Note the DB defaults themselves are still wrong and worth a migration; the write path no longer depends on them.

### Changed
- **Every view in the app has a URL.** The whole authed app used to live at `/dashboard`, with navigation held in a `page` string in React state and two more state variables standing in for the detail views. Nothing was linkable, reload always dumped you back on the dashboard, and "back" from a detail page was hardcoded to the list regardless of where you came from. There are now 19 real routes — 14 landlord, 5 tenant portal — with `renderPage()`, the `page` state, and the hand-rolled `history.pushState` shim all deleted. Records are addressable: `/properties/[propertyId]`, `/tenants/[tenantId]`.
  - **Naming:** URLs name resources, not the UI's current label for them. The `contracts` table is served at `/leases` because that is the domain word and the nav label; the To Do List is `/maintenance` because the tenant portal calls the same resource Maintenance and a copy change must not need a redirect. Payment Reminders sits at `/settings/reminders`, not under `/email`, because it is a different feature over a different table from Email Automation.
  - **Roles are a routing boundary.** `(landlord)` (a route group, invisible in URLs) and `portal/` have separate guards; a landlord hitting `/portal` is redirected to `/dashboard` and vice versa. Both share one store, mounted above them.
  - **Params are UUIDs, deliberately.** Every href comes from `lib/routes.js` and every param resolves through the `find*` helpers there, so adding slug columns later changes one file instead of ~20 call sites. Slugs are not possible today anyway: only `parking_spots` has a human-readable unique key, and `units` lacks even `UNIQUE (property_id, unit_number)`.
  - Soft navigation does **not** refetch — verified at zero Supabase requests across all 12 sidebar destinations.
  - Sidebar links carry `prefetch={false}`: all 12 are in the viewport at load, and each prefetch is an RSC request that runs middleware, which calls `supabase.auth.getUser()` — a network round trip, not a local decode.
- **The `?next=` param now carries the path you actually asked for.** Gating moved into `middleware.js`, which is the only layer that can see the pathname; the `(app)` layout hardcoded `?next=/dashboard`, harmless with one URL and a real bug with twenty. The layout's own check remains the authoritative one. Login's post-auth default is now role-aware, reusing the role probe it already ran and threw away. **Adding a new top-level app segment means adding it to `APP_PREFIXES` in `middleware.js`.**
- The sidebar highlights the right item on detail pages. Active state derives from `usePathname()` by longest-matching prefix, so `/properties/<id>` lights Properties (the old `currentPage === item.id` check left nothing lit) and `/portal/payments/history` lights History rather than both it and Payments.
...
### Removed
- `App`, `renderPage()`, `Sidebar`, `LangToggle` and the `history.pushState`/`popstate` shim from `property-management-app.jsx`, which is now a component library with no default export. The shim was a back-button emulator that wrote an empty URL and overwrote `window.history.state` — which the App Router uses for its own state, so it had to go the moment real routing arrived. The monolith is down from 5,613 to ~4,500 lines, with `T` (652 lines) now in `lib/i18n/strings.js` so the sidebar and route shells can import strings without pulling the monolith into their module graph.
- `TenantLeasePage` is deliberately **not** routed. It was reachable in `renderPage` but had no nav entry and no caller — dead code. A URL is a promise that something is there, so giving it one would have shipped a feature by accident. It stays unexported pending a decision to finish or delete it.
- `app/(app)/dashboard/layout.js` and `app/(app)/dashboard/page.js` — replaced by `app/(app)/layout.js` (guard + store) and `app/(app)/(landlord)/dashboard/page.js`. `/dashboard` still resolves, so existing bookmarks keep working.
